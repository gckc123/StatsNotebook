            ...
            return (
                <div>
                    ...
                    {(currentPanel === "LRPanel") &&
                        <div className="NMACheckbox"><Checkbox size="small"
                        disabled = {this.props.Variables.RandomEffect.length > 0}
                        ...
                        onClick={(event) => this.props.updateAnalysisSettingCallback(event, currentPanel,"robustReg")}/>Robust Regression
                            <StyledTooltip title="Robust regression is not available for mixed model (when random effects are present). It should be noted that robust regression might not always produce a convergent solution.">
                            <span className="pl-2"><FontAwesomeIcon icon={faInfoCircle} size="1x"/></span></StyledTooltip>
                        </div>
                    }
